...
def main(*args, **kwargs):
    ...
    args.add_argument('--dataset', type=str, default='91-IMAGE', help='specified dataset name(as described in config file')
    ...
    args.add_argument('--output_color', type=str, default='RGB', choices=('RGB', 'L', 'GRAY', 'Y'), help='output color mode')
    ...
    with Environment(model, f'{save_root}/save', f'{save_root}/log', feature_index=model.feature_index, label_index=model.label_index) as env:
        ...
        env.fit(args.batch, args.epochs, dataset, restart=args.retrain, learning_rate=args.lr,
                learning_rate_schedule=lr_decay('stair', args.lr, decay_step=args.lr_decay_step, decay_rate=args.lr_decay))
        if args.test:
