...
Created Date: 2020-5-39

TF2.0 executing environment
"""
import logging
...
def _parse_ckpt_name(name):
    if not name:
        return 0
    _, epochs = Path(name).stem.split('-')
    return int(epochs)


class Env:
    """Keras model runtime Env-ironment.

    Args:
        model: a Model object representing a container of multiple Module objects.
    ...
    def _startup(self):
        if isinstance(self._saved, Path):
            ...
            self.ckpt = tf.train.Checkpoint(
                **self.model.modules, **self.model.opts)
