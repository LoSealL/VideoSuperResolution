...
VSR Backend resolver, supports:
- pytorch >= 1.0.0
- tensorflow <= 1.15.0 (regard as `tensorflow`)
- tensorflow >= 2.0.0 (experimental, regard as `keras`)
"""
import logging
import os
...
if BACKEND in ('tensorflow', 'keras'):
    try:
        ...
        tf_ver_major, tf_ver_minor, _ = [
            int(s) for s in tf.__version__.split('.')]
...
if BACKEND == 'pytorch':
    try:
        ...
        if _ver[0] != '1' or _ver[1] <= '1':
            LOG.warning(f"[!] PyTorch version too low: {torch.__version__},"
                        " recommended 1.2.0")
    except ImportError:
        LOG.fatal("[!] PyTorch package not found in your system.")
        raise ImportError("Not an available backend found!"
