...
class DRSR(SuperResolution):
    ...
    def gen_noise(self, inputs, ntype, max1=0.06, max2=0.16):
        with tf.name_scope('GenNoise'):
            ...
            if ntype == 'gaussian':
                ...
                irr = Noise.tf_camera_response_function(
                    inputs, icrf[i], max_val=1)
                noise = Noise.tf_gaussian_poisson_noise(
                    irr, max_c=max1, max_s=max2)
                img = Noise.tf_camera_response_function(
                    irr + noise, crf[i], max_val=1)
                return img - inputs
            else:
    ...
    def build_graph(self):
        ...
        with tf.variable_scope('Offset'):
            ...
            if not self.auto:
                for i in range(4):
                    shift.append(Noise.tf_gaussian_noise(
                        inputs_norm, n.offset[i]))
                var_shift = []
            else:
                for i in range(4):
                    shift.append(self.noise_shift(
                        inputs_norm, 8, f'NoiseShift_{i}'))
        ...
        def loss_fn2():
            w = self.weights
            tv_noise = [tf.reduce_mean(
                tf.image.total_variation(x)) for x in shift]
