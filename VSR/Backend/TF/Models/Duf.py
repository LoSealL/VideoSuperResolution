...
class DUF(SuperResolution):
    ...
    def _dyn_filter3d(self, x, F):
        ...
        x_localexpand = tf.nn.conv2d(x, filter_localexpand_np, [1, 1, 1, 1],
                                     'SAME')  # b, h, w, 1*5*5
        x_localexpand = tf.expand_dims(
            x_localexpand, axis=3)  # b, h, w, 1, 1*5*5
        ...
        with tf.variable_scope(self.name):
            ...
            for i in range(self.layers - 3):
                x = tf.layers.batch_normalization(
                    t, training=self.training_phase)
                x = tf.nn.relu(x)
            ...
            f = self.conv3d(f, np.prod(self.filter_size)
                            * np.prod(self.scale), 1)
            ds_f = tf.shape(f)
            ...
            for i in range(self.channel):
                x = self._dyn_filter3d(
                    input_norm[:, self.depth // 2, ..., i], f[:, 0])
    ...
    def build_loss(self):
        ...
        self.metrics['psnr'] = tf.reduce_mean(
            tf.image.psnr(label_c, sr_norm, 1))
