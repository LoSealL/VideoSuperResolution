    ...
    def build_loss(self):
        with tf.name_scope('Loss'):
            ...
            with tf.variable_scope(self.name, reuse=True):
                gp = gradient_penalty(
                    *self.g_outputs, graph_fn=self.D, lamb=10)
            d_loss += gp
            self._build_loss(g_loss, d_loss)
...
class RGAN(GAN):
    def build_loss(self):
        with tf.name_scope('Loss'):
            g_loss, d_loss = loss_relative_bce_gan(
                *self.d_outputs, average=False)
            self._build_loss(g_loss, d_loss)
...
class RLSGAN(GAN):
    def build_loss(self):
        with tf.name_scope('Loss'):
            g_loss, d_loss = loss_relative_lsgan(
                *self.d_outputs, average=False)
            self._build_loss(g_loss, d_loss)
