...
PyTorch VSR executing environment
"""
import logging
...
def _parse_ckpt_name(name):
    if not name:
        return 0
    _, epochs = Path(name).stem.split('.')[0].split('_')
    return int(epochs[2:])
...
class Env:
    ...
    def _startup(self):
        ...
        if isinstance(self._logd, Path):
            ...
            if _logger.isEnabledFor(logging.DEBUG):
                fd = logging.FileHandler(
                    self._logd / 'vsr_debug.log', encoding='utf-8')
                fd.setFormatter(
    ...
    def _find_last_ckpt(self, pattern):
        ...
        if not isinstance(self._saved, Path):
            return
        ckpt = sorted(self._saved.glob(pattern),
            ...
            if fp:
                ...
                try:
                    model.load_state_dict(torch.load(
                        str(fp), map_location=map_location))
    ...
    def _save_model(self, step, path):
        if not isinstance(path, Path):
            return
        for key, model in self.model.modules.items():
            fp = path / _make_ckpt_name(key, step)
            torch.save(model.state_dict(), str(fp))
        for key, opt in self.model.opts.items():
            fp = path / f'{key}.pth'
            torch.save(opt.state_dict(), str(fp))

    def _save_model_permanently(self, step, every_n_step=500):
        if not isinstance(self._saved, Path):
            return
        root = self._saved / 'snapshot'
        root.mkdir(exist_ok=True, parents=False)
        if step % every_n_step == 0:
            self._save_model(step, root)
