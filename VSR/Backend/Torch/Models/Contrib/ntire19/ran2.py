...
class ST_Block5(nn.Module):
    def __init__(self, num_unit, nChannel, prev=0,
        ...
        self.Infuse = nn.Conv2d(nChannel * (prev + 1), nChannel, kernel_size=1,
                                padding=0, bias=bias)
        if prev > 0:
            self.Conv = nn.Conv2d(nChannel * (2), nChannel,
