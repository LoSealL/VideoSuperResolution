...
from ..Util import pad_if_divide, psnr, upsample
from .Model import SuperResolution
from .Ops.Blocks import RB, EasyConv2d
from .Ops.Loss import VggFeatureLoss, ragan_bce_loss
from .Ops.Motion import STN, Flownet
...
class TecoDiscriminator(nn.Module):
    def __init__(self, channel, filters, patch_size):
        ...
        self.conv3 = EasyConv2d(
            f, f * 2, 4, 2, activation='leaky', use_bn=True)
        self.conv4 = EasyConv2d(
            f * 2, f * 4, 4, 2, activation='leaky', use_bn=True)
...
class TeCoGAN(SuperResolution):
    ...
    def __init__(self, scale, channel, weights, vgg_layers, vgg_layer_weights,
        ...
        if self.use_gan:
            ...
            self.dopt = torch.optim.Adam(
                self.trainable_variables('dnet'), 5e-5)
    ...
    def prepare_dnet_input(self, frames):
        ...
        for i in range(1, len(frames) - 1):
            ...
            with torch.no_grad():
                ...
                cube.append(
                    torch.cat((pre, cur, nex, warpf, cur, warpb), dim=1))
        return cube
    ...
    def train(self, inputs, labels, learning_rate=None):
        ...
        # Generator loss
        # 1. Image MSE
        loss_image_mse = torch.stack(
            [F.mse_loss(x, y) for x, y in zip(sr, labels)])
        # 2. Ping-pong loss
