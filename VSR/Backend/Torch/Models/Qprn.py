...
from ..Util import pad_if_divide, psnr
...
class Fnet(nn.Module):
    def __init__(self, channel, L=2, gain=64):
        ...
        self.flownet = nn.Sequential(
            ...
            Rdb(64, 64), Rdb(64, 64), Upsample(64, 4),
    ...
    def train(self, inputs, labels, learning_rate=None):
        ...
        for lq, label in zip(frames[1:], labels[1:]):
            ...
            for his, his_hq in zip(history[:-1], history_hq[:-1]):
                ...
                his_flow = self.qprn.flownet(
                    his_to_warp + [hq.detach()], his_to_warp)
                his_flow = [t.squeeze(1) for t in his_flow.split(1, dim=1)]
                his_warp = self.qprn.warpper(
                    torch.stack(his_to_warp, dim=1), *his_flow)
            ...
            l2_warp = 0.8 * F.mse_loss(lb_warp, label) + \
                0.2 * F.mse_loss(lq_warp, lq)
            tv_flow = total_variance(torch.stack(flow, dim=1), dims=(2, 3))
            loss = self.w[0] * l1_image + self.w[1] * \
                l2_warp + self.w[2] * tv_flow
...
class _Trainer(SRTrainer):
    ...
    def fn_benchmark_each_step(self, label=None, feature=None, name=None,
        ...
        with torch.set_grad_enabled(False):
            outputs, metrics = self.model.eval(
                [feature], [label], epoch=v.epoch)
