...
class SRFEAT(SuperResolution):
    def __init__(self, channel, scale, patch_size=64, weights=(1, 0.01, 0.01),
        ...
        if self.use_gan:
            # vanilla image
            self.dnet1 = DCGAN(channel, np.log2(
                patch_size // 4) * 2, scale, 'bn')
            self.dopt1 = torch.optim.Adam(
                self.trainable_variables('dnet1'), 1e-4)
        if self.use_feat_gan:
            # vgg feature
            self.dnet2 = DCGAN(256, np.log2(patch_size // 16) * 2, scale, 'bn')
            self.dopt2 = torch.optim.Adam(
                self.trainable_variables('dnet2'), 1e-4)
