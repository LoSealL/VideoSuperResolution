...
class CascadeRdn(nn.Module):
    def __init__(self, channels, filters, depth=3, use_ca=False,
        ...
        for i in range(self.depth[0]):
            ...
            setattr(self, f'rdn_{i}', Rdb(
                channels, filters, self.depth[1], **kwargs))
            if use_ca:
...
class HighPassFilter(nn.Module):
    def __init__(self, recursions=1, kernel_size=9, stride=1, include_pad=True,
        ...
        self.filter_low = LowPassFilter(recursions=1, kernel_size=kernel_size,
                                        stride=stride, include_pad=include_pad)
        self.recursions = recursions
