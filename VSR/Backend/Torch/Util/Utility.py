...
def upsample(img, scale, border='reflect'):
    ...
    assert c is not None, "img must define channel number"
    c = int(c)
    filters = [torch.reshape(torch.eye(c, c), [c, c, 1, 1])
               * k for k in kernels]
    weights = torch.stack(filters, dim=0).transpose(
        0, 1).reshape([-1, c, 5, 5])
...
def imfilter(image: torch.Tensor, kernel: torch.Tensor, padding=None):
    with torch.no_grad():
        ...
        assert image.dim(
        ) == 4, f"Dim of image must be 4, but is {image.dim()}"
        ...
        assert kernel.dim(
        ) == 3, f"Dim of kernel must be 3, but is {kernel.dim()}"
        ...
        for i, k in zip(image.split(1), kernel.split(1)):
            ...
            if padding is None:
                ret.append(F.conv2d(i, _k, padding=[
                           x // 2 for x in kernel.shape[1:]]))
