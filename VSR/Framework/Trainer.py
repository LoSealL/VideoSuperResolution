...
class FRVSR(Trainer):
    """Specialized trainer for FRVSR(frame recurrent vsr)"""
        ...
        for epoch in range(self.last_epoch + 1, epochs + 1):
            ...
            with tqdm.tqdm(train_iter, unit='batch', ascii=True) as r:
                for label, feature, name in r:
                    ...
                    for fn in label_callbacks:
                        label = fn(label, name=name)
                    clips = np.split(feature, train_loader.depth, axis=1)
                    last_frame = clips[0]
                    last_sr = np.zeros_like(label)[:, 0]
                    for this_frame in clips:
                        feature = [this_frame, last_frame, last_sr]
                        loss = self._m.train_batch(feature, label, learning_rate=lr, epochs=epoch)
                        outputs = self._m.test_batch(feature)
                        last_sr = outputs[0]
    ...
    def infer(self, loader, config, **kwargs):
        ...
        for feature, _, name in tqdm.tqdm(it, 'Infer', ascii=True):
            for fn in feature_callbacks:
                feature = fn(feature, name=name)
            clips = np.split(feature, loader.depth, axis=1)
            last_frame = clips[0]
            sr_shape = feature.shape
            sr_shape[2] *= config.scale
            sr_shape[3] *= config.scale
            last_sr = np.zeros(sr_shape, dtype='float32')[:, 0]
            for this_frame in clips:
                outputs = self._m.test_batch([this_frame, last_frame, last_sr])
                last_sr = outputs[0]
                last_frame = this_frame
                for fn in output_callbacks:
                    outputs = fn(outputs, input=feature, mode=loader.color_format, name=name)
    ...
    def benchmark(self, loader, config, **kwargs):
        ...
        for label, feature, name in tqdm.tqdm(it, 'Test', ascii=True):
            ...
            last_sr = np.zeros_like(label)[:, 0]
            for this_frame in clips:
                outputs, metrics = self._m.test_batch([this_frame, last_frame, last_sr], label, epochs=epoch)
                outputs = self._m.test_batch([this_frame, last_frame, last_sr])
                last_sr = outputs[0]
                last_frame = this_frame
                for k, v in metrics.items():
