...
class Environment:
  ...
  def fit(self,
    ...
    train_loader = QuickLoader(batch, dataset, 'train', self.model.scale,
                               steps_per_epoch,
                               crop='random', augmentation=augmentation,
                               **kwargs)
    val_loader = QuickLoader(batch, dataset, 'val', self.model.scale,
                             validate_numbers, crop='center', **kwargs)
  ...
  def test(self, dataset, **kwargs):
    ...
    loader = QuickLoader(1, dataset, 'test', self.model.scale, -1, crop=None,
                         **kwargs)
    ...
    for img in tqdm.tqdm(it, 'Test', ascii=True):
      ...
      for fn in self.output_callbacks:
        outputs = fn(outputs, input=img[self.fi], label=img[self.li],
  ...
  def predict(self, files, mode='pil-image1', depth=1, **kwargs):
    ...
    loader = QuickLoader(1, data, 'test', self.model.scale, -1, crop=None,
                         **kwargs)
    it = loader.make_one_shot_iterator()
    if len(it):
      print('===================================')
    ...
    for img in tqdm.tqdm(it, 'Infer', ascii=True):
      ...
      for fn in self.output_callbacks:
        outputs = fn(outputs, input=img[self.fi], label=img[self.li],
                     mode=loader.color_format, name=name)
