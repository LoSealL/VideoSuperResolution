...
from VSR.DataLoader.VirtualFile import ImageFile, RawFile, _ALLOWED_RAW_FORMAT
from VSR.Util import ImageProcess, Utility
...
class Loader(object):
  ...
  def __init__(self, dataset, method, loop=False):
    ...
    self.random = dataset.random and not (
        method == 'test')  # random crop, or gridded crop
    ...
    if self.mode.lower() == 'pil-image1':
      ...
      self.dataset = [RawFile(
        fp, dataset.mode, (dataset.width, dataset.height), loop) for fp in
        dataset_file]
  ...
  def _build_iter(self):
    while True:
      ...
      for frames_hr, frames_lr, x, y, name in self.grid:
        ...
        crop_lr = [img.crop(box // [*self.scale, *self.scale]) for img in
                   frames_lr]
  ...
  def build_loader(self, crop=True, **kwargs):
    ...
    self.patch_size = Utility.shrink_mod_scale(self.patch_size,
    ...
    for vf in self.dataset:
      ...
      for _ in range(vf.frames // depth):
        frames_hr = [ImageProcess.shrink_to_multiple_scale(img,
                                                           self.scale) if self.modcrop else img
                     for img in
                     vf.read_frame(depth)]
        frames_lr = [ImageProcess.imresize(img, np.ones(2) / self.scale) for img
                     in frames_hr]
    ...
    if self.random:
      ...
      for hr, lr, name in self.frames:
        ...
        self.grid += [(hr, lr, _x, _y, name) for _x, _y in
...
class BatchLoader:
  ...
  def __init__(self,
    ...
    if convert_to.lower() in ('gray', 'l'):
      ...
      tf.logging.warning(
        f'Unknown format {convert_to}, use grayscale by default')
      self.color_format = 'L'
...
class EpochIterator:
  ...
  Args:
      loader: A `BasicLoader` or `BatchLoader` to provide properties.
  ...
  def __next__(self):
    ...
    while self.grids and len(batch_hr) < self.batch:
      ...
      crop_lr = [ImageProcess.crop(img, box // [*self.scale, *self.scale]) for
                 img in lr]
      ops = np.random.randint(0, 2, [3]) if self.aug else [0, 0, 0]
    ...
    if isinstance(batch_hr, np.ndarray) and isinstance(batch_lr, np.ndarray):
      try:
        return np.squeeze(batch_hr, 1), np.squeeze(batch_lr, 1), np.squeeze(
          batch_name)
      except ValueError:
...
class QuickLoader:
  ...
  BasicLoader concurrently pre-fetches clips into memory every n iterations,
  and provides several methods to select clips. Unlike BatchLoader, BasicLoader
  ...
  def _parse_config(self, config, **kwargs):
    ...
    _needed_args = (
      'batch', 'depth', 'patch_size', 'scale', 'steps_per_epoch', 'convert_to')
  ...
  def _vf_gen_lr_hr_pair(self, vf, depth, index):
    ...
    frames_lr = [ImageProcess.imresize(img, np.ones(2) / self.scale) for img in
                 frames_hr]
  ...
  def size(self):
    ...
    return np.sum(
      [np.prod((*vf.shape, vf.frames, bpp), dtype=np.uint64) for vf in
       self.file_objects])
  ...
  def make_one_shot_iterator(self, memory_usage=None, shard=1, shuffle=False):
    ...
    grids = self._generate_crop_grid(fr, self.patches_per_epoch,
                                     shuffle=shuffle)
