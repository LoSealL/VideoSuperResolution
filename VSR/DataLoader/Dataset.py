    ...
    def _get_dataset(desc, use_as_video=None, name=None):
        ...
        for i in desc:
            ...
            if use_as_video:
                ...
                hr_pattern = [x if x not in all_path else all_path[x]
                              for x in hr]
                lr_pattern = [x if x not in all_path else all_path[x]
                              for x in lr]
            hr_data = Dataset(root).include(
                *(_extend_pattern(x) for x in hr_pattern))
    ...
    with open(describe_file, 'r') as fd:
        ...
        for name in all_path:
            if '[video]' in name:
                ...
                datasets[name] = _get_dataset(
                    Config(test=all_path[name]), name=name)
        return datasets
