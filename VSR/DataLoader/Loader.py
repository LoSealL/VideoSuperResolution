...
class EpochIterator:
    ...
    def __init__(self, loader, shape, steps, shuffle=None, caching=False):
        ...
        self.index = []
        for i in range(t):
            depth = self.depth if self.depth >= 0 else len(
                self.loader.data['hr'][i])
            idx_ = [(i, np.array([j + x for x in range(depth)])) for j in
                    range(-(depth // 2), frame_nums[i] - (depth // 2))]
            d2_ = depth // 2
            r_ = frame_nums[i] - depth + 1
            self.index += idx_ if temporal_padding or d2_ == 0 else idx_[
                d2_:d2_ + r_]
    ...
    def __next__(self):
        ...
        slc = slice(self.count * self.shape[0],
                    (self.count + 1) * self.shape[0])
...
class Loader(object):
    ...
    def set_color_space(self, target: str, mode: str):
        if not mode.upper() in ('RGB', 'L', 'YCbCr', 'Gray'):
            raise ValueError(
                f"Invalid mode: {mode}, must be RGB | L | YCbCr | Gray")
        assert target.lower() in ('hr', 'lr')
    ...
    def prefetch(self, shuffle=None, memory_usage=None):
        ...
        if not self.fs:
            ...
            if available_mem > self.aux['cap']:
                ...
                for i in range(self.threads):
                    self.fs.append(self.thp.submit(self._prefecth_chunk, n, i))
